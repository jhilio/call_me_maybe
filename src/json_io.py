...
class Parameter(BaseModel):
    type: str = Field(..., min_length=1,
                      description="Type must be a non-empty string")
...
def get_func_def(input_path: str) -> list[dict]:
    """open the path in argument read it,
        and verify each function has correct format
...
def get_prompt(input_path: str) -> list[str]:
    """open the path in argument read it,
        and verify each prompt has correct format
