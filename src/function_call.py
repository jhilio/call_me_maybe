...
from restrained_decoding import (free_commentary, phrase_only_rd,
                                 param_fill_rd, restrained_decoding_number)
...
Fill the parameter according to function definition and\
the user request.<|im_end|>
<|im_start|>user
{prompt}<|im_end|>
"""
                    ),
...
""")
    ...
    def __init__(self, llm: MyLLM, func_data: list[dict], prompt: str):
        """simply initialise the class
            with given and default value where needed"""
...
""")
    ...
    def get_param(self, max_try: int = 3) -> None:
        ...
        if self.function_name == "None":
            return
        try:
            func = [func for func in self.func_data
                    if func["name"] == self.function_name][0]
        except IndexError:
            self.parameter = {}
            return
        ...
        for param_name, param_info in param_schema.items():
            ...
            prompt_template = self.param_type_prompts.get(
                param_type, self.param_type_prompts["string"])
            ...
            for i in range(max_try):
                full_prompt = base_prompt + retry_context + (
                    "<|im_start|>assistant\n" if param_type != "string"
                    else "parameter value:")

                param_try = self.set_param(
                    param_type,
                    param_name,
                    full_prompt,
                    random=bool(retry_context))
                if self.judge_param(param_try, param_name, func):
                    break
                if i < max_try - 1:
                    print(f"temporary result : {param_try}")
                    print("retriying..\033[5m.\033[0m")
                    act_prompt = self.COMMENTARY_PROMPT.format(
                            param_try=param_try,
                            param_name=param_name,
                            func=func,
                            prompt=self.prompt,
    ...
    def judge_param(
            self, param_value: Any, param_name: str, func: dict) -> bool:
...
"B" if this candidate is a repeat of previous parameter or \
doenst fit the user request<|im_end|>
<|im_start|>user
{self.prompt}<|im_end|>
<|im_start|>assistant
"""
        ...
        if phrase_only_rd(
                judge_prompt, ["A", "B"], self.llm, max_token=True) == "B":
    ...
    def set_param(self,
        ...
        Args:
            param_type (str):
                type of data the parameter will be,
                used to choose correct llm restraining function
            param_name (str):
                name of the param being set
            full_prompt (str):
                prompt that will be used by the llm
            random (bool, optional):
                weither the decoding sshould use argmax or weighted probability
                Defaults to False.

        Returns:
            str | bool | int | float | None:
                the value found by the llm, none only in case of error
        """
        result: str = ""
        if param_type == "number":
            numbers_in_prompt = [x for x in extract_numbers(
                self.prompt, allow_float=True)
                if float(x) not in self.parameter.values()]
            value_output = restrained_decoding_number(
                full_prompt, numbers_in_prompt, self.llm)
            try:
            ...
            typed_number = [float(x) if '.' in x else int(x)
                            for x in numbers_in_prompt]
            left_in_prompt = [str(x) for x in typed_number
                              if x not in self.parameter.values()
                              and isinstance(x, int)]
            value_output = restrained_decoding_number(
                full_prompt, left_in_prompt, self.llm)
            try:
                return int(value_output)
            except Exception:
                return None
                ...
                if param_name == name:
                    for k, v in bias.items():
                        if k in self.prompt:
                            for phrases in v:
                                boosted.append(
                                    self.llm.encode(phrases).tolist()[0])
            ...
            while result:
                result = result.strip(" \n")
                if ((result[0] == '"' or final_prompt[-1] == '"')
                        and result[-1] == '"'):
                    result = result.strip('"')
                elif ((result[0] == "'" or final_prompt[-1] == "'")
                        and result[-1] == "'"):
                    result = result.strip("'")
                else:
                    break
